...
import { calculateAvailableUsdc, withdrawalStatusLabel, type WithdrawalMethod, type WithdrawalStatus } from "@/lib/withdrawals";
...
type WithdrawalRow = { id: string; amount_usdc_micro: number | string; method: WithdrawalMethod; destination_address: string | null; bank_details: BankDetails | null; status: WithdrawalStatus; payout_tx_hash: string | null; payout_reference: string | null; rejection_reason: string | null; created_at: string };

function WithdrawalDestination({ item }: { item: WithdrawalRow }) {
  if (item.method === "BANK" && item.bank_details) return <div className="mt-2 text-xs text-muted-foreground"><p className="font-semibold text-foreground">Depósito bancario · {item.bank_details.currency}</p><p>{item.bank_details.accountHolder} · {item.bank_details.holderId}</p><p>{item.bank_details.bankName} · {item.bank_details.accountType} · {item.bank_details.accountNumber}</p>{item.payout_reference ? <p className="mt-1">Referencia: {item.payout_reference}</p> : null}</div>;
  return <div className="mt-2"><p className="text-xs font-semibold text-foreground">Retiro en USDC</p><p className="break-all font-mono text-xs text-muted-foreground">{item.destination_address}</p>{item.payout_tx_hash ? <p className="mt-1 break-all font-mono text-xs text-muted-foreground">Tx: {item.payout_tx_hash}</p> : null}</div>;
}
...
export default async function WalletPage() {
  ...
  const [{ data: paymentData, error: paymentError }, { data: withdrawalData, error: withdrawalError }, { data: organizationData }] = await Promise.all([
    ...
    supabase.from("withdrawal_requests").select("id,amount_usdc_micro,method,destination_address,bank_details,status,payout_tx_hash,payout_reference,rejection_reason,created_at").eq("organization_id", organization.id).order("created_at", { ascending: false }),
