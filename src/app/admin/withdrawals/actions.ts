...
const actionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("paid"), requestId: z.string().uuid(), txHash: z.string().trim().regex(/^0x[0-9a-fA-F]{64}$/) }),
]);
...
export async function reviewWithdrawalAction(formData: FormData) {
  ...
  const { data: current, error: readError } = await admin.from("withdrawal_requests").select("id,organization_id,status,amount_usdc_micro").eq("id", parsed.requestId).single();
  ...
  const update = parsed.action === "approve"
    ...
    : parsed.action === "reject"
      ...
      : { status: "PAID", payout_tx_hash: parsed.txHash.toLowerCase(), paid_at: now, updated_at: now };
