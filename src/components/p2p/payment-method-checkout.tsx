...
export function PaymentMethodCheckout(props: PaymentSessionCheckoutProps) {
  ...
  return <div>
    <div className="mb-4 rounded-xl bg-warning/15 p-3 text-xs leading-5 text-warning-foreground">Prueba en Base Sepolia · Utiliza únicamente el comerciante de prueba de P2P.me. No transfieras pesos reales.</div>
    {method === "ars" ? <>
      <h2 className="mb-3 text-lg font-semibold">Pagar en pesos argentinos</h2>
      <PesoCheckout {...props} />
    </> : <>
      ...
      <div className="mt-4 grid gap-3">
        <button type="button" onClick={() => setMethod("ars")} className="rounded-2xl border-2 border-primary bg-secondary/40 p-4 text-left transition hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
          ...
          <span className="mt-3 block text-xs font-medium text-primary">Sin conectar wallet</span>
        </button>
